...
prealloc_head *prealloc_init(unsigned int init_cells,
	...
	head->avail_arr =
		(unsigned int **)calloc(num_cells_y, sizeof(unsigned int *));
	head->avail_arr[0] =
		(unsigned int *)calloc(num_cells_x, 2*sizeof(unsigned int));
...
prealloc_cell *prealloc_new(prealloc_head *head) {
	...
	if ( head->avail_cell[0] == -1U) {
		...
		//	cell->data = &((cell_data **)head->data)[head->full_next[1]]
		//		[head->full_next[0]];
		...
		head->avail_cell[0] =	head->avail_arr[cell->place[1]][2*cell->place[0]];
		head->avail_cell[1] =	head->avail_arr[cell->place[1]][2*cell->place[0]+1];
	}
...
void prealloc_del(prealloc_head *head, prealloc_cell *cell) {
	...
	head->avail_arr[cell->place[1]][2* cell->place[0]] = head->avail_cell[0];
	head->avail_arr[cell->place[1]][2* cell->place[0] +1] = head->avail_cell[1];
...
void prealloc_realloc(prealloc_head *head) {
	...
	head->avail_arr[head->full_next[1]] =
		(unsigned int *)calloc(head->init_cells, 2*sizeof(unsigned int));
